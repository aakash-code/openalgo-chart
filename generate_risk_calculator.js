...
export function calculateRiskPosition(params: RiskCalculationParams): RiskCalculationResult {
    ...
    // The Pine Script uses riskAmount / (stopPct / 100) -> riskAmount / ((slPoints / entryPrice)) 
    // Position Value = riskAmount / (slPoints / entryPrice)
    // Quantity = Position Value / entryPrice = riskAmount / slPoints
    // Same thing!
    const quantity = Math.floor(riskAmount / slPoints);

    if (quantity <= 0) return { error: 'Calculated quantity is 0. Increase capital or risk %' };
...
`
fs.writeFileSync('src/utils/indicators/riskCalculator.ts', content);
